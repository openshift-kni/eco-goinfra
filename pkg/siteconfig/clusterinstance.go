...
func (builder *CIBuilder) WithExtraLabels(key string, labels map[string]string) *CIBuilder {
	...
	if key == "" {
		glog.V(100).Infof("checking the key of clusterinstance extraLabels is empty")
	...
	if labels == nil {
		glog.V(100).Infof("checking the extraLabels of clusterinstance map values are nil")

		builder.errorMsg = "can not apply if extraLabels map values are nil"
