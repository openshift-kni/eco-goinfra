...
const (
	...
	defaultPRNamespace       = "test-cluster-ns"
...
func TestNewPRBuilder(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			templateVersion: defaultPRTemplateVersion,
			client:          true,
			expectedError:   "provisioningRequest 'nsname' cannot be empty",
		},
		{
			name:            defaultPRName,
			nsname:          defaultPRNamespace,
	...
	for _, testCase := range testCases {
		...
		testBuilder := NewPRBuilder(
			testSettings, testCase.name, testCase.nsname, testCase.templateName, testCase.templateVersion)
...
func TestPullPR(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			addToRuntimeObjects: true,
			client:              true,
			expectedError:       fmt.Errorf("provisioningRequest 'nsname' cannot be empty"),
		},
		{
			name:                defaultPRName,
			nsname:              defaultPRNamespace,
			...
			expectedError: fmt.Errorf(
				"provisioningRequest object %s does not exist in namespace %s", defaultPRName, defaultPRNamespace),
		},
		{
			name:                defaultPRName,
			nsname:              defaultPRNamespace,
	...
	for _, testCase := range testCases {
		...
		if testCase.addToRuntimeObjects {
			runtimeObjects = append(runtimeObjects, buildDummyPR(defaultPRName, defaultPRNamespace))
		}
		...
		testBuilder, err := PullPR(testSettings, testCase.name, testCase.nsname)
...
func TestPRGet(t *testing.T) {
	...
	for _, testCase := range testCases {
		...
		if testCase.expectedError == "" {
			...
			assert.Equal(t, testCase.testBuilder.Definition.Namespace, provisioningRequest.Namespace)
		} else {
...
func buildDummyPR(name, namespace string) *provisioningv1alpha1.ProvisioningRequest {
...
func buildTestClientWithDummyPR() *clients.Settings {
	return clients.GetTestClients(clients.TestClientParams{
		K8sMockObjects: []runtime.Object{
			buildDummyPR(defaultPRName, defaultPRNamespace),
		},
		SchemeAttachers: provisioningTestSchemes,
	})
}
...
func buildValidPRTestBuilder(apiClient *clients.Settings) *ProvisioningRequestBuilder {
	return NewPRBuilder(apiClient, defaultPRName, defaultPRNamespace, defaultPRTemplateName, defaultPRTemplateVersion)
}
