...
func NewPRBuilder(
	apiClient *clients.Settings, name, nsname, templateName, templateVersion string) *ProvisioningRequestBuilder {
	glog.V(100).Infof(
		"Initializing new ProvisioningRequest structure with the following params: name: %s, nsname: %s", name, nsname)
	...
	if nsname == "" {
		glog.V(100).Info("The namespace of the ProvisioningRequest is empty")

		builder.errorMsg = "provisioningRequest 'nsname' cannot be empty"
...
func PullPR(apiClient *clients.Settings, name, nsname string) (*ProvisioningRequestBuilder, error) {
	glog.V(100).Infof("Pulling existing ProvisioningRequest %s under namespace %s from cluster", name, nsname)
	...
	if nsname == "" {
		...
		return nil, fmt.Errorf("provisioningRequest 'nsname' cannot be empty")
	}

	if !builder.Exists() {
		glog.V(100).Info("The ProvisioningRequest %s does not exist in namespace %s", name, nsname)

		return nil, fmt.Errorf("provisioningRequest object %s does not exist in namespace %s", name, nsname)
	}
...
func (builder *ProvisioningRequestBuilder) Get() (*provisioningv1alpha1.ProvisioningRequest, error) {
	...
	glog.V(100).Infof(
		"Getting ProvisioningRequest object %s in namespace %s", builder.Definition.Name, builder.Definition.Namespace)
	...
	if err != nil {
		glog.V(100).Infof(
			"Failed to get ProvisioningRequest object %s in namespace %s: %v",
...
func (builder *ProvisioningRequestBuilder) Exists() bool {
	...
	glog.V(100).Infof(
		"Checking if ProvisioningRequest %s exists in namespace %s", builder.Definition.Name, builder.Definition.Namespace)
...
func (builder *ProvisioningRequestBuilder) Create() (*ProvisioningRequestBuilder, error) {
	...
	glog.V(100).Infof(
		"Creating ProvisioningRequest %s in namespace %s", builder.Definition.Name, builder.Definition.Namespace)
...
func (builder *ProvisioningRequestBuilder) Update() (*ProvisioningRequestBuilder, error) {
	...
	glog.V(100).Infof(
		"Updating ProvisioningRequest %s in namespace %s", builder.Definition.Name, builder.Definition.Namespace)

	if !builder.Exists() {
		glog.V(100).Infof(
			"ProvisioningRequest %s does not exist in namespace %s", builder.Definition.Name, builder.Definition.Namespace)
...
func (builder *ProvisioningRequestBuilder) Delete() error {
	...
	glog.V(100).Infof(
		"Deleting ProvisioningRequest %s in namespace %s", builder.Definition.Name, builder.Definition.Namespace)

	if !builder.Exists() {
		glog.V(100).Infof(
			"ProvisioningRequest %s in namespace %s does not exist",
