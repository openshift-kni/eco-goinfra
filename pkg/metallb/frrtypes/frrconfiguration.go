...
type Neighbor struct {
	...
	// +kubebuilder:validation:XValidation:message="connect time should be between 1 seconds to 65535",
	// rule="duration(self).getSeconds() >= 1 && duration(self).getSeconds() <= 65535"
	// +kubebuilder:validation:XValidation:message="connect time should contain a whole number of seconds",
...
type Advertise struct {
	...
	// Allowed is the list of prefixes allowed to be propagated to
...
// AllowedInPrefixes represents a list of allowed prefixes to receive from the given neighbor.
...
// AllowedOutPrefixes represents a list of allowed prefixes to send to a given neighbor.
...
// AllowMode +kubebuilder:validation:Enum=all;filtered.
type AllowMode string

const (
	// AllowAll enables AllowMode to receive all incoming routes.
	AllowAll AllowMode = "all"
	// AllowRestricted enables allow AllowMode to receive a specific set of incoming routes.
...
func (in *FRRConfiguration) DeepCopyObject() runtime.Object { //nolint:ireturn
