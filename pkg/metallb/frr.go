...
func NewFrrConfigurationBuilder(
	...
	if net.ParseIP(peerIP) == nil {
		glog.V(100).Infof("The peerIP of the frrConfiguration bgp peer contains invalid ip address %s", peerIP)
...
func (builder *FrrConfigurationBuilder) WithToReceiveModeAll() *FrrConfigurationBuilder {
	if valid, _ := builder.validate(); !valid {
		return builder
	}
	...
	builder.Definition.Spec.BGP.Routers[0].Neighbors[0].ToReceive.Allowed.Mode = frrtypes.AllowMode("all")

	return builder
}

// WithBlockAllMode blocks all incoming routes.
func (builder *FrrConfigurationBuilder) WithBlockAllMode() *FrrConfigurationBuilder {
	if valid, _ := builder.validate(); !valid {
		return builder
	}
...
// WithToReceiveModeFiltered defines the password used between BGP peers to sync.
func (builder *FrrConfigurationBuilder) WithToReceiveModeFiltered(neigh []frrtypes.Neighbor, prefixes []string) *FrrConfigurationBuilder {
	if valid, _ := builder.validate(); !valid {
		return builder
	}
	...
	for _, prefix := range prefixes {
		...
		if _, _, err := net.ParseCIDR(prefix); err != nil {
			glog.V(100).Infof("The prefix of the frrConfiguration BGP peer contains an invalid IP address: %s", prefix)
	...
	builder.Definition.Spec.BGP.Routers[0].Neighbors = neigh

	// Assign the list of valid prefixes
	neigh[0].ToReceive.Allowed.Prefixes = make([]frrtypes.PrefixSelector, len(prefixes))
	for i, prefix := range prefixes {
		neigh[0].ToReceive.Allowed.Prefixes[i] = frrtypes.PrefixSelector{Prefix: prefix}
	}
...
func (builder *FrrConfigurationBuilder) WithEBGPMultiHop() bool {
	if valid, _ := builder.validate(); !valid {
		return false
	}
...
func (builder *FrrConfigurationBuilder) WithConnectTime(connectTime metav1.Duration) *FrrConfigurationBuilder {
	...
	glog.V(100).Infof(
		"Define hold time. The bgp connection will attempt every %s to establish a connection with the BGP Peer", connectTime)
