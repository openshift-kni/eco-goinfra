...
func Pull(apiClient *clients.Settings, name, nsname string) (*Builder, error) {
	...
	if !builder.Exists() {
		return nil, fmt.Errorf("clusterLogging oject %s doesn't exist in namespace %s", name, nsname)
	}
...
func (builder *Builder) Update(force bool) (*Builder, error) {
	...
	if err != nil {
		if force {
			glog.V(100).Infof("Failed to update the clusterLogging object %s in namespace $s. "+
			...
			if err != nil {
				glog.V(100).Infof(
					"Failed to update the clusterLogging object %s in namespace $s."+
