...
import (
	...
	"github.com/rh-ecosystem-edge/eco-goinfra/pkg/sriov/sriovtypes"
...
type PoolConfigBuilder struct {
	...
	Definition *sriovtypes.SriovNetworkPoolConfig
	// Created sriovNetworkPoolConfig object.
	Object *sriovtypes.SriovNetworkPoolConfig
...
func (builder *PoolConfigBuilder) Create() (*PoolConfigBuilder, error) {
	...
	if !builder.Exists() {
		unstructuredSriovPoolConfig, err := runtime.DefaultUnstructuredConverter.ToUnstructured(builder.Definition)
		if err != nil {
			glog.V(100).Infof("Failed to convert structured SriovNetworkPoolConfig to unstructured object")

			return nil, err
		}

		unsObject, err := builder.apiClient.Resource(
			GetSriovPoolConfigGVR()).Namespace(builder.Definition.Namespace).Create(
			context.TODO(), &unstructured.Unstructured{Object: unstructuredSriovPoolConfig}, metav1.CreateOptions{})

		if err != nil {
			glog.V(100).Infof("Failed to create SriovNetworkPoolConfig")
...
func (builder *PoolConfigBuilder) Delete() error {
	...
	err := builder.apiClient.Resource(
		GetSriovPoolConfigGVR()).Namespace(builder.Definition.Namespace).Delete(
...
func (builder *PoolConfigBuilder) Get() (*sriovtypes.SriovNetworkPoolConfig, error) {
	if valid, err := builder.validate(); !valid {
		return nil, err
	}
	...
	unsObject, err := builder.apiClient.Resource(
		GetSriovPoolConfigGVR()).Namespace(builder.Definition.Namespace).Get(
		context.TODO(), builder.Definition.Name, metav1.GetOptions{})

	if err != nil {
		glog.V(100).Infof(
			"Failed to Unmarshal SriovNetworkPoolConfig: unstructured object to structure in namespace %s",
			builder.Definition.Name, builder.Definition.Namespace)
...
func (builder *PoolConfigBuilder) Update() (*PoolConfigBuilder, error) {
	...
	_, err = builder.apiClient.Resource(
		GetSriovPoolConfigGVR()).Namespace(builder.Definition.Namespace).Update(
		context.TODO(), &unstructured.Unstructured{Object: unstructuredSriovPoolConfig}, metav1.UpdateOptions{})

	if err != nil {
		glog.V(100).Infof("Failed to update SriovNetworkPoolConfig")
...
// GetSriovPoolConfigGVR returns SriovNetworkPoolConfig's GroupVersionResource which could be used for Clean function.
func GetSriovPoolConfigGVR() schema.GroupVersionResource {
	return schema.GroupVersionResource{
		Group: "sriovnetwork.openshift.io", Version: "v1", Resource: "sriovnetworkpoolconfigs",
	}
}
...
func (builder *PoolConfigBuilder) convertToStructured(
	unsObject *unstructured.Unstructured) (*sriovtypes.SriovNetworkPoolConfig, error) {
	poolConfig := &sriovtypes.SriovNetworkPoolConfig{}

	err := runtime.DefaultUnstructuredConverter.FromUnstructured(unsObject.Object, poolConfig)
	if err != nil {
		glog.V(100).Infof(
			"Failed to convert from unstructured to SriovNetworkPoolConfig object in namespace %s",
			builder.Definition.Name, builder.Definition.Namespace)
