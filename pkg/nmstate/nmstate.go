...
func (builder *Builder) Delete() (*Builder, error) {
	...
	glog.V(100).Infof("Deleting the NMState object %s",
		builder.Definition.Name,
	)

	if !builder.Exists() {
		return builder, fmt.Errorf("NMState cannot be deleted because it does not exist")
	}
...
func PullNMstate(apiClient *clients.Settings, name string) (*Builder, error) {
	...
	if !builder.Exists() {
		return nil, fmt.Errorf("NMState oject %s doesn't exist", name)
	}
